...
def run(st_id, df=df):
        ...
        for att in atts:
            ...
            for root_file in root_files:
                ...
                for freq_band in freq_bands:
                    cpevents, cptimes, cpvolts = get_eventsvoltstraces(basic_read_root(root_dir, selectors = selector), band_pass=1, pulse_filter=1, freq_band_filter=freq_band)
                    if len(cpevents) == 0:
                        continue
                    ...
                    new_data = pd.DataFrame({
                        ...
                        'freq_band': [freq_band] * len(snr_list.keys()),
    ...
    df.to_csv(f'../data/pulser-data_st{station_id}.csv', index=False)
