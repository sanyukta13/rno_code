...
for file in os.listdir(input_dir):
    ...
    data = {ch: {'ws': [], 'impulsivity': [], 'RPR': [], 'entropy': [], 'kurtosis': [], 'noise': [], 'time': []} for ch in plt_chs}
    ...
    for event in reader.run():
        ...
        for ch in plt_chs:
            if station.has_channel(ch):
                ...
                # data[ch]['SNR'].append(channel[chp.SNR])
                data[ch]['impulsivity'].append(channel[chp.impulsivity])
                data[ch]['RPR'].append(channel[chp.root_power_ratio])
                data[ch]['entropy'].append(channel[chp.entropy])
                data[ch]['kurtosis'].append(channel[chp.kurtosis])
    ...
    for i, ch in enumerate(sorted(plt_chs)):
        ...
        # ax[i].scatter(data[ch]['time'], data[ch]['SNR'], c=data[ch]['ws'], cmap=cmap, norm=norm, marker='o',
        #              s=30, edgecolors='black', linewidths=0.5)
        ax[i].scatter(data[ch]['time'], data[ch]['impulsivity'], c=data[ch]['ws'], cmap=cmap, norm=norm, marker='*',
                     s=50, edgecolors='black', linewidths=0.5)
        ax[i].scatter(data[ch]['time'], data[ch]['RPR'], c=data[ch]['ws'], cmap=cmap, norm=norm, marker='s',
                     s=30, edgecolors='black', linewidths=0.5)
        ax[i].scatter(data[ch]['time'], data[ch]['entropy'], c=data[ch]['ws'], cmap=cmap, norm=norm, marker='>',
                     s=30, edgecolors='black', linewidths=0.5)
        ax[i].scatter(data[ch]['time'], data[ch]['kurtosis'], c=data[ch]['ws'], cmap=cmap, norm=norm, marker='<',
                     s=30, edgecolors='black', linewidths=0.5)        
        ...
        ax[i].set_ylabel('AU', fontsize=11)
    ...
    plt.savefig(os.path.join(output_dir, f'stats_{file[:-13]}.png'), dpi=150, bbox_inches='tight')
    plt.close()
    print(f'Saved: {file}')
